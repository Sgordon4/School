...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
	...
	public BagInterface<T> union(BagInterface<T> anotherBag){
		@SuppressWarnings("unchecked")
		T[] union = (T[])new Object[this.getCurrentSize() + anotherBag.getCurrentSize()];

		System.arraycopy(this.toArray(), 0, union, 0, this.getCurrentSize());
		System.arraycopy(anotherBag.toArray(), 0, union, this.getCurrentSize(), anotherBag.getCurrentSize());

		return new ResizableArrayBag<T>(union);
	}
	...
	public BagInterface<T> intersection(BagInterface<T> anotherBag){
		...
		for(int i = 0; i < this.getCurrentSize(); i++) {
			...
			if(!intersection.contains(bag[i])) {	//Checks if the item has already been added to our new bag

				//Finds the overlap in the specific item
				frequency = Math.min(this.getFrequencyOf(bag[i]), anotherBag.getFrequencyOf(bag[i]));	
	...
	public BagInterface<T> difference(BagInterface<T> anotherBag){
		...
		for(int i = 0; i < this.getCurrentSize(); i++) {
			...
			if(!difference.contains(bag[i])) {	//Checks if the item has already been added to our new bag

				//Finds the number of this specific item that don't match up between bags
				frequency = Math.abs(this.getFrequencyOf(bag[i]) - anotherBag.getFrequencyOf(bag[i]));
				for(int a = 0; a < frequency; a++) {
	...
	public void removeEvery(T anEntry) {
		...
		for(int i = 0; i < frequency; i++) {
			removeEntry(getIndexOf(anEntry));	//Could just use remove(anEntry);
