...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display Size
        //Convert the last part of the buffer into an int, keeping endianness in mind
        unsigned int l = buffer[off+31-3] | (buffer[off+31 - 2] << 8) | (buffer[off+31 - 1] << 16) | (buffer[off+31] << 24);
        printf("%d\n", l);
    }
