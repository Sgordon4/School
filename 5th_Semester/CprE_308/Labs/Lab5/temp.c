...
void shortest_remaining_time(process *proc)
{
    //Fill in some of the rest of proc's information
    for(int i = 0; i < NUM_PROCESSES; i++){
        proc[i].remainingtime = proc[i].runtime;
        proc[i].flag = 0;
    }
...
void round_robin(process *proc)
{
    ...
    int i = 0;
    int mostRecentlyArrived = 0;

    while (finished < NUM_PROCESSES) // while there are still unfinished processes
    {
        //Find the most recently arrived
        for(int j = 0; j < NUM_PROCESSES; j++){
            if(sorted[j].proc.arrivaltime <= time){
                mostRecentlyArrived == j;
            }
        }
        //Reset loop if needed
        if(i == mostRecentlyArrived+1)
            i = 0;
